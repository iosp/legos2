...
class importActions extends sfActions {
	...
	{		 
		...
		if ($handle) {
			...
			fwrite ( $handle, "[" . date ( 'Y-m-d H:i:s' ) . "]  Blf Name is " . $blfName . "\n\r" );
			fclose ( $handle );
		}				
		...
		if ($this->getRequest ()->hasFiles ()) 
		{
			...
			{	
				...
				{	 
					...
					$mission = $import->import ( $file['name'] );
					$mission->setBlfName($blfName);
					$mission->save();
					...
					return $this->renderText(json_encode(array(
							...
							'RedirectUrl' => 'import/show?missionId=' . $missionIdImported . '&fileName=' . $file['name'] 
					)));
				}
				else 
				{
